...
public class FencedKataCodeBlockParser extends AbstractBlockParser {
    ...
    final private static Pattern KATA_CLOSING_FENCE = Pattern.compile("^(?:`{3,}|~{3,})\\{\\{execute\\}\\}(?=[ \t]*$)");

    final private FencedKataCodeBlock block = new FencedKataCodeBlock();
    ...
    public FencedKataCodeBlockParser(DataHolder options, char fenceChar, int fenceLength, int fenceIndent, int fenceMarkerIndent) {
    ...
    public BlockContinue tryContinue(ParserState state) {
        ...
        if (matches) {
            ...
            matcher = KATA_CLOSING_FENCE.matcher(trySequence);
            if (matcher.find()) {
                int foundFenceLength = matcher.group(0).length();
                ...
                if (foundFenceLength >= fenceLength) {
                    // closing fence - we're at end of line, so we can finalize now
                    block.setClosingMarker(trySequence.subSequence(0, foundFenceLength));
                    block.setType(KataCodeType.execute);
                    return BlockContinue.finished();
                }
            }
        }
        ...
        public BlockStart tryStart(ParserState state, MatchedBlockParser matchedBlockParser) {
            ...
            if (state.getIndent() < 4) {
                ...
                if ((matcher = OPENING_FENCE.matcher(trySequence)).find()) {
                    ...
                    FencedKataCodeBlockParser blockParser = new FencedKataCodeBlockParser(state.getProperties(), fenceChar, fenceLength, state.getIndent(), nextNonSpace);
