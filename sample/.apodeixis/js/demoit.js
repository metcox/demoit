...
class WebTerm extends BaseHTMLElement {
    static get styles() {
        ...
        #terminal {
            ...
            display: flex;
            background-color: hsl(197 67% 12%);
        }
        ...
        .xterm-viewport {
            scrollbar-color: hsl(197 67% 24% / 1) hsl(197 67% 0% / 1);
        }
        ...
        .xterm-viewport::-webkit-scrollbar-track {
          background: hsl(197 67% 0% / 1);
          border-radius: 100vw;
          margin-block: 0.5em;
        }

        .xterm-viewport::-webkit-scrollbar-thumb {
           background: hsl(197 67% 24% / 1);
           border-radius: 100vw;
         }
    ...
    addTab() {
        ...
        let window = this.shadowRoot.appendChild(div.lastChild);
        ...
        let ws = new WebSocket("ws://127.0.0.1:8080/shellSocket/"+ this.path);

        let term = new Terminal({
            fontFamily: 'MesloLGS NF',
            fontSize: 20,
            theme: {background:'rgb(10,39,50)'}
        });
        let fitAddon = new FitAddon.FitAddon();
        term.loadAddon(fitAddon);
        term.onData(command => {
            ws.send(
                action('TERMINAL_COMMAND', {command})
            );
        });
        ws.onopen = () => {
            ws.send(action('TERMINAL_INIT'));
            ws.send(action('TERMINAL_READY'));
            term.open(window.querySelector('#terminal'));
            fitAddon.fit();
        };
        ...
        term.onResize(e => {
            console.log('resizing term to ' + e.cols + ':' + e.rows);
            let cols = e.cols;
            let rows = e.rows;
            ws.send(
                action('TERMINAL_RESIZE', {cols, rows})
            );
        });
